...
"""
serialize(stream::IO, value::T) where {T} = write(stream, bytes(build!(value)))
...
"""
deserialize(stream::IO, ::Type{T}) where {T} = read(T, read(stream))
...
numslots(soff::AbstractVector{<:Integer}) = ((soff[end] - 4) ÷ 2) + 1
numslots(::Type{T}) where {T} = numslots(slot_offsets(T))
...
function FlatBuffers.read(::Type{T}, buffer::AbstractVector{UInt8}, pos::Integer) where {T}
    FlatBuffers.read(Table(T, buffer, pos))
end
function FlatBuffers.read(b::Builder{T}) where {T}
    FlatBuffers.read(Table(T, b.bytes[b.head+1:end], get(b, b.head, Int32)))
end
...
function has_identifier(::Type{T}, bytes) where {T}
    length(bytes) ≥ 8 && String(bytes[5:8]) == FlatBuffers.file_identifier(T)
end
...
function buildbuffer!(b::Builder{T1}, arg::T, prev=nothing) where {T1<:Any, T<:Any}
    ...
    # build a struct type with provided `arg`
        ...
        # TODO the below is still suspicious
        ...
        while isdefault && i > 0
            i -= 1
            isdefault = getfieldvalue(arg, i) == default(T, i+1)
        end
        soff = slot_offsets(T)
